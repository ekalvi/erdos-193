...
🌐 **Visual introduction:** [erdos.q5m.ai](https://erdos.q5m.ai) — the proof
...
| `design/padic_macrocycle_lift.py`, `design/verify_padic_macrocycle_lift.py`, `design/test_padic_macrocycle_runtime.py` | Exact `8 -> 16 -> 8` affine/3-adic chart, bounded lift census, active-row resume checkpoints, durable JSONL run logs, interval-correct progress accounting, and independent mathematical/runtime verification (`padic-macrocycle-lift-summary.json`); isolates the reachable-birth proximity target, not an unconditional tail theorem |
| `design/PADIC-REACHABLE-SECANT-DEPTH-AUDIT.md`, `design/padic_reachable_secant_depth_audit.py`, `design/verify_padic_reachable_secant_depth_audit.py` | Hostile source/history and exact-algebra audit of the proposed reachable-secant q-depth; stops at condition 6, with signed checkpoint/resume support and hashed summary/witness artifacts |
| `design/PADIC-LATENT-FAMILY-MACROCYCLE-AUDIT.md`, `design/padic_latent_family_macrocycle_audit.py`, `design/verify_padic_latent_family_macrocycle_audit.py` | Focused source-to-transition audit of the fixed latent family; independently regenerates the 66,429 bounded residue edges and stops at condition 2 because they are not a line-token induction certificate; signed `[OpenAI Codex]` |
| `design/latent_family_reachable_entry_audit.py`, `design/verify_latent_family_reachable_entry_audit.py` | Independent exact proof that the fixed latent `N`-orbit cannot enter any continuation of the canonical guarded-L5 parent that preserves the same two-cone birth guard; includes recorded L5 -> L6 and all-alternate-word-at-recorded-prefix censuses (`latent-family-reachable-entry-audit-summary.json`), but does not prove successor availability or unguarded exclusion |
| `design/guarded_cone_core_audit.py`, `design/verify_guarded_cone_core_audit.py` | Exact guarded-history theorem freezing every two-cone secant to one of the 246 inherited parent-pair lineages; closes both known positive-depth latent families under that guard, but not effects outside the cones or successor availability |
| `design/OBSERVED-RETURNING-LINE-EXPERIMENT.md`, `design/observed_returning_line_experiment.py`, `design/verify_observed_returning_line_experiment.py` | Bounded exact L5–L8 returning-line census with canonical Pluecker provenance, separate carriage/cursor traces, machine-readable table and minimal witnesses; zero coverage by the 66,429-edge family and no decreasing implemented rank |
...
| `viz/` | The website ([erdos.q5m.ai](https://erdos.q5m.ai)) |
| `REPORT.md` | **The full research log** — every result, refutation and correction, in order |

## Current proof status

The terminal-steered Hilbert lift gives an explicit infinite counterexample:
one selected Hilbert index per block of sixteen, gaps between 4 and 28, a fixed
finite step menu in `ℤ³`, and no collinear ordered triple. The complete theorem
is `Hilbert193.erdos193_unconditional`; `formal/Hilbert193/Hilbert193/AxiomAudit.lean`
reports only Mathlib's standard axioms. The paper source is
`paper/erdos193.tex`, and `viz/hilbert-proof.pdf` is the compiled preprint.
External mathematical review and community acceptance remain pending.

### Archived earlier route
