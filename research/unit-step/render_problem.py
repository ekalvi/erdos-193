...
def main():
    ...
    try:
        ...
        basis, steps, triples = displays
        basis = basis.replace(r"e_d", r"e_{d_*}")
        ...
        text(.07, .805, r"Find the least positive integer $d_*$ admitting an infinite sequence")
        text(.07, .770, r"$P_n\in\mathbb{Z}^{d_*}$. The allowed steps are the positive standard basis vectors:")
        text(.50, .725, "$" + basis + "$", ha="center", fontsize=19)
        text(.07, .680, r"Each $e_r$ has a $1$ in coordinate $r$ and zeros in every other coordinate.", fontsize=13.5)
        text(.07, .615, "1. Start at the origin:", weight="bold")
        text(.50, .570, r"$P_0=0.$", ha="center", fontsize=21)
        text(.07, .505, "2. Take positive unit steps:", weight="bold")
        text(.50, .460, "$" + steps + "$", ha="center", fontsize=20)
        text(.07, .395, "3. No three vertices are collinear:", weight="bold")
        text(.50, .350, "$" + triples + "$", ha="center", fontsize=19)
        text(.07, .290, r"Coordinate sums equal $n$, so the last condition expresses noncollinearity.", fontsize=12.5)
        text(.07, .215, r"Goal: prove existence at $d_*$ and impossibility in every smaller dimension.", fontsize=13.5)
