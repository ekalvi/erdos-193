...
projects linearly onto the original walk, so is itself admissible. Thus
$\E(3,s)\Rightarrow\B(s)$. The existing $g_{85}$ six-step construction gives
the \emph{working bounds}
\[
 \boxed{4\le d_*\le s_*\le6}
 \qquad\text{subject to independent verification of the $g_{85}$ proof.}
\]
Its written infinite argument is intended to be unconditional; the caveat
concerns pending independent review, not an additional mathematical hypothesis.
The established fallback ceiling is $s_*\le16$, from the original
sixteen-step 3D construction. The lower bound follows because every ternary
word of length eight contains an abelian square.
The converse implication $\B(s)\Rightarrow\E(3,s)$ is \emph{not assumed}.
Equivalently, $d_*$ is the minimum number of step types when the ambient
lattice dimension is unrestricted. Subject to verification of the six-step
proof, the candidate pairs are $(4,4),(4,5),(4,6),(5,5),(5,6),(6,6)$;
none is asserted as the answer.
